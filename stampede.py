...
def create_grid(rows, cols, custom_layout=None):
    grid = [[Cell('exit' if custom_layout and custom_layout[i][j] == 'E' else 
                  'obstacle' if custom_layout and custom_layout[i][j] == 'X' else 
                  'walkable') 
...
def run_simulation(grid, steps=10):
    ...
    for _ in range(steps):
        ...
        # We could possibly remove this to let the simulation run automatically if we want
...
def main():
    ...
    grid_selection = input("Type '1' to use a pre-made grid, or '2' to create your own grid: ")
    if grid_selection.lower() == '1':
        ...
        custom_layout = []
        print("Enter your grid layout line by line (use '0' for walkable, 'X' for obstacle, 'E' for exit): ")
        for _ in range(rows):
            new_row = input()
            if (len(new_row) == rows):
    ...
    walkable_count = sum(1 for row in grid for cell in row if cell.cellType == 'walkable')
    density = input_safe("Enter population density as a percentage (0-100): ", lambda: random.randint(0, 100))

    # These are technically percentage chances to be this trait, which should be fine (I think)
    rational = input_safe("Enter the percentage that is rational (0-100): ", lambda: random.randint(0, 100))
    strong = input_safe("Enter the percentage that is strong (0-100): ", lambda: random.randint(0, 100))
    relaxed = input_safe("Enter the percentage that is relaxed (0-100): ", lambda: random.randint(0, 100))
    ...
    max_steps = input_safe("Enter the number of simulation steps to take (any integer): ", lambda: 100)
    ...
    for _ in range(total_people):
        while True:
            ...
            if grid[i][j].cellType == 'walkable' and not grid[i][j].is_occupied():
                ...
                grid[i][j].occupied = Person(is_strong, is_rational, is_relaxed, grid[i][j])
                break
